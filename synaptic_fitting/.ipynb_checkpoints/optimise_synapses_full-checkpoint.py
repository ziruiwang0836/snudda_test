...
class OptimiseSynapsesFull(object):
    ...
    def load_trace_data(self, data_file):
        ...
        with open(data_file, "r") as f:
            ...
            baseline_samples = int(baseline_duration_ms * self.sample_freq / 1000)

            baseline_value = np.mean( self.volt[0:baseline_samples])*1000  # in mV
            print(f"Baseline voltage: {baseline_value:.4f} (mV)")
    ...
    def plot_data(self,
        ...
        return self.time[t_idx], self.volt[t_idx], t_plot[t2_idx], v_plot[t2_idx]
    ...
    def find_trace_heights(self, time, volt, peak_idx):
        ...
        v_base = np.mean(volt[int(0.3 * peak_idx[0]):int(0.8 * peak_idx[0])])  # Baseline before first peak
        ...
        for idx_b in range(1, len(peak_idx)):
            # initial guess for decay fit a,b,c
            if peak_height[0] > 0:
                ...
                if idx_b < len(peak_idx) - 1:
                    ...
                    p0d = [-1e-5, 1e5, -0.0798]
            # select the time window between peaks to fit decay
            ...
            try:
                ...
                try:
                    fit_params, pcov = scipy.optimize.curve_fit(decay_func, t_ab_fit, v_ab_fit) # fit decay with each decay segment
                except:
                    import traceback
                    tstr = traceback.format_exc()
                    self.write_log(tstr)
                    ...
                    p0d[1] *= 0.01
                    fit_params, pcov = scipy.optimize.curve_fit(decay_func, t_ab_fit, v_ab_fit, p0=p0d) # if its still not working, try again with smaller decay constant

                t_b = time[peak_idx_b] - t_ab[0]
                v_base_b = decay_func(t_b, fit_params[0], fit_params[1], fit_params[2]) # esimate baseline at next peak time

                peak_height[idx_b] = volt[peak_idx_b] - v_base_b # calculate peak height relative to estimated baseline
        ...
        return peak_height.copy(), decay_fits, v_base #can plot the peak_heights
    ...
    def neuron_synapse_helper_glut(self, t_spike,
        ...
        decay_error8 = np.sum((smooth_exp_trace8[idx_max8:] - sim_trace8[idx_max8:]) ** 2) \
                        / (self.num_smoothing - idx_max8 + 1) * 1000

        decay_error9 = np.sum((smooth_exp_trace9[idx_max9:] - sim_trace9[idx_max9:]) ** 2) \
                        / (self.num_smoothing - idx_max9 + 1) * 1000
    ...
    def sobol_scan(self,
        ...
        try:
            ...
            idx = 0
            error_list = [] # For plotting errors
            ...
            for u, tau_r, tau_f, tau_ratio, cond \
                ...
                self.write_log(f'------------sobol_scan_trail {idx} error: {error}, params: {param_this_run} (u, tau_r, tau_f, tau_ratio, cond)-------------')
                ...
                #     self.write_log("Early stopping Sobol scan: no improvement")
                #     break

            
            # plot the error progression
            cum_best = np.minimum.accumulate(error_list)
            plt.scatter(np.arange(len(error_list)),cum_best)
            plt.plot(cum_best)
            plt.xlabel("Number of parameter evaluations")
            plt.ylabel("Best error found so far")
    ...
    def parallel_optimise_single_cell(self, n_trials=10000, post_opt=False):
        ...
        if post_opt:
            print("------Starting post-optimisation refinement...------")
    ...
    def get_refined_parameters(self):
        ...
        def callback(xk):
            ...
            err = func(xk)        # current error value
            learning_curve.append(err)
            print(f"------------Iteration {len(learning_curve)} | Error = {err:.6g}----------")
